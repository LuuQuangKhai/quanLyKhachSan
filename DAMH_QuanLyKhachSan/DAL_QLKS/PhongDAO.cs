...
namespace DAMH_QuanLyKhachSan.DAL_QLKS
{
    class PhongDAO
    {
        ...
        public bool AddPhong (string tenPH, string MaLoaiPH)
        {
            try
            {
                help.executeNonQuery("sp_InsertNewPhong '" + tenPH + "', '" + MaLoaiPH.Trim()+"'");
                return true;
            }
            catch(Exception ex)
            {
        ...
        public bool UpdatePhong(string tenPH, string MaLoaiPH, string MAPH)
        {
            try
            {
                help.executeNonQuery("update PHONG set TENPH = '" + tenPH + "', MALOAIPHONG = '" + MaLoaiPH.Trim() + "' where MaPH = '" + MAPH.Trim() + "'");
                return true;
            }
            catch(Exception ex)
            {
